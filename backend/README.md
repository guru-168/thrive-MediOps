...
This service is containerised (`backend/Dockerfile`) rather than deployed
via a platform's default Python runtime. That is deliberate:
`app/ml/model.joblib` is a pickled scikit-learn/XGBoost pipeline and
`requirements.txt` pins the exact library versions it was created with, so
the serving environment has to be reproducible. The image also runs a
model self-check at build time, which turns a bad version combination into
a failed build instead of a 500 in production.

Any container host works (Railway, Render, Fly.io, Cloud Run). Build
context is `backend/`:

```bash
docker build -t mediops-api ./backend
...
|---|---|---|
| `ALLOWED_ORIGINS` | **yes** | The frontend's exact origin, e.g. `https://mediops.vercel.app`. Without it, CORS defaults to localhost only and the browser blocks every call. |
| `ALLOWED_ORIGIN_REGEX` | optional | Anchored regex for Vercel preview URLs, e.g. `^https://mediops-[a-z0-9-]+\.vercel\.app# Patient Follow-up Risk Predictor API (PS-01)
...
the artifact at `MODEL_PATH`.
 ...
 |
| `PORT` | injected | Set automatically by most hosts; the image binds `0.0.0.0:$PORT`. |

`MODEL_PATH` should be left unset - the default is absolute and derived
from the package location, so it is correct regardless of working
directory. Thresholds (`RISK_THRESHOLD_LOW/HIGH`, `INTERVENTION_THRESHOLD`)
default to the model's calibrated values and only need setting to override
them.
...
# expect: {"status":"ok", ..., "model_type":"trained_model","model_loaded":true}
```

`model_type` must read `trained_model`. If it reads `rule_based`, the
model artifact was not found at `MODEL_PATH` and the API has fallen back
to the interim scorer.
